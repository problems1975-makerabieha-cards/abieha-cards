...
def apply_effect(r, card):
    ...
    if card["n"] == "+2":
        ...
        r["log"].insert(
            0,
            f"العقوبة الآن: اسحب {r['pendingDraw2']} أو رد +2 / عكس / تخطي بنفس اللون"
        )
        r["turn"] = next_index(r)
        return

    # +4: تتراكم العقوبة وتنتقل للاعب التالي
    if card["n"] == "+4":
        r["pendingDraw4"] = int(r.get("pendingDraw4", 0)) + 4
        r["pendingDraw2"] = 0
        r["log"].insert(
            0,
            f"العقوبة الآن: اسحب {r['pendingDraw4']} أو رد +4 / عكس / تخطي بنفس اللون"
        )
        r["turn"] = next_index(r)
        return
    ...
    if card["n"] in ["عكس", "تخطي"]:
        ...
        # اللعب العادي: عكس/تخطي لا يفتح عقوبة +2
        if card["n"] == "عكس":
            r["direction"] *= -1
